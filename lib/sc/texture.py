...
# Neue Imports für plattformübergreifende Tool-Ausführung
from lib.platform_detect import detect_os, OperatingSystem, is_wine_available
from lib.config import find_tool, get_config
...
class SWFTexture(Writable):
    ...
    def load_khronos_texture(self, data):
        """Lädt KTX-Texturen mit PVRTexToolCLI (plattformübergreifend)"""
        ...
        try:
            run_pvr_tex_tool(inputPath, outputPath)
            ...
            print(colorama.Fore.RED + "[CRITICAL] Missing PVRTexToolCLI")
            os_type = detect_os()
            if os_type == OperatingSystem.MACOS:
                print(colorama.Fore.YELLOW + "[TIP] Download PVRTexToolCLI from:")
                print("      https://developer.imaginationtech.com/pvrtextool/")
                print("      Place it in lib/bin/macos/")
            return
        except ToolExecutionError as e:
            print(colorama.Fore.RED + f"[ERROR] PVRTexToolCLI failed: {e}")
            return
        except Exception as e:
            print(colorama.Fore.RED + "[ERROR] Unexpected error:", e)
            return

    def load_sctx_texture(self, path, externalTextureFilepath):
        """Lädt SCTX-Texturen mit SctxConverter (plattformübergreifend)"""
        ...
        try:
            run_sctx_converter("decode", path, out, transparent=True)
            print(colorama.Fore.LIGHTMAGENTA_EX + "[INFO] SCTX decoded to:", os.path.basename(out))
            self._image = Image.open(out)
            
        except ToolNotFoundError:
            print(colorama.Fore.RED + "[CRITICAL] Missing SctxConverter")
            os_type = detect_os()
            if os_type == OperatingSystem.MACOS:
                print(colorama.Fore.YELLOW + "[TIP] Install Wine: brew install wine-stable")
                print("      Or compile SctxConverter for macOS (see BUILD_MACOS.md)")
            return
        except ToolExecutionError as e:
            print(colorama.Fore.RED + f"[ERROR] SctxConverter failed: {e}")
            return  
        except Exception as e:
            print(colorama.Fore.RED + "[ERROR] Unexpected error:", e)
            return
    ...
    def load(self, swf, tag, has_external_texture):
        ...
        if externalTextureFilepath:
            ...
            if ext == "zktx":
                ...
                self.load_sctx_texture(path, externalTextureFilepath)
            return
